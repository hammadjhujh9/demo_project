...
export default function FinanceOfficerScreen({ navigation }: FinanceOfficerScreenProps) {
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        vouchersList.push({
          id: doc.id,
          imageUrl: data.receiptUrl, // Map receiptUrl to imageUrl for consistency
          ...data
        });
      });
      
      console.log('Fetched vouchers:', vouchersList);
  ...
  const approveReceipt = async (receiptId: string, comment: string) => {
    ...
    try {
      setIsLoading(true);
      const voucherRef = doc(db, 'vouchers', receiptId);
      ...
      await updateDoc(voucherRef, {
        status: 'approved',
        approvedBy: userData.name || auth.currentUser?.email || 'Unknown',
      ...
      Alert.alert("Success", "Voucher has been approved");
    } catch (error) {
  ...
  const rejectReceipt = async (receiptId: string, reason: string) => {
    ...
    try {
      ...
      Alert.alert("Success", "Voucher has been rejected");
    } catch (error) {
      console.error("Error rejecting voucher:", error);
      Alert.alert("Error", "Failed to reject voucher. Please try again.");
    } finally {
      setIsLoading(false);
      setShowCommentModal(false);
      setCommentText('');
  ...
  // Add this helper function for permissions
  ...
  // For custom header buttons we need to create a custom right component
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <AppHeader 
        ...
        customRightComponent={renderHeaderRightComponent()}
      />
      ...
      {isLoading && !refreshing ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            {selectedFilter !== 'all' 
              ? `No ${selectedFilter} vouchers at the moment` 
              : 'No vouchers to display yet'}
          </Text>
        </View>
      ) : (
        <ScrollView 
...
const styles = StyleSheet.create({
  ...
  // Remove header, headerLeft, headerRight, title, subtitle styles
  // Add new style for custom header buttons
