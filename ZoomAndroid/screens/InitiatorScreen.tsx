...
type InitiatorScreenNavigationProp = NativeStackNavigationProp<RootStackParamList, 'InitiatorScreen'>;

interface InitiatorScreenProps {
  navigation: InitiatorScreenNavigationProp;
}

export default function InitiatorScreen({ navigation }: InitiatorScreenProps) {
  // Tab state
  ...
  const uploadProofOfPayment = async () => {
    ...
    try {
      ...
      setUploadError(`Upload process failed: ${error.message || 'Unknown error'}`);
    } finally {
  ...
  const shareFile = async (fileUri: string) => {
    try {
      ...
      await Share.shareAsync({ url: fileUri });
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* Using the new AppHeader component */}
      <AppHeader 
        title="Payment Initiator"
                ...
                <View style={styles.summaryRow}>
                  ...
                  <Text style={styles.summaryValue}>
                    ${selectedVoucher?.amount ? 
                      (typeof selectedVoucher.amount === 'number' ? 
                  ...
                  <View style={styles.detailSection}>
                    ...
                    <View style={styles.detailItem}>
                      ...
                      <Text style={styles.detailItemValue}>
                        ${typeof selectedVoucher.amount !== 'undefined' ? 
                          (typeof selectedVoucher.amount === 'number' ? 
                            selectedVoucher.amount.toLocaleString() : 
                            selectedVoucher.amount) : 
                          '0'}
