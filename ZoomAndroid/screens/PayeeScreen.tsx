...
type AdminScreenProps = {
  navigation: NativeStackNavigationProp<any>; // Replace 'any' with your stack param list if you have one
};
...
export default function AdminScreen({ navigation }: AdminScreenProps) {
  ...
  const [bankFormErrors, setBankFormErrors] = useState({} as any);
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      const vouchersList: Receipt[] = [];
      
      querySnapshot.forEach((doc) => {
        vouchersList.push({
          id: doc.id,
          imageUrl: doc.data().receiptUrl, // Note: mapping receiptUrl to imageUrl
          ...doc.data()
        } as Receipt);
      });
      
      setReceipts(vouchersList);
    } catch (error) {
      console.error("Error fetching vouchers:", error);
      Alert.alert("Error", "Could not load vouchers. Please try again.");
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
  ...
  const validateBankDetails = () => {
    ...
    if (!bankDetails.accountTitle.trim()) {
    ...
    if (!bankDetails.amount.trim()) {
      errors.amount = 'Amount is required';
    } else if (isNaN(Number(bankDetails.amount)) || Number(bankDetails.amount) <= 0) {
  ...
  const submitBankDetailsAndUpload = () => {
    if (!validateBankDetails() || !selectedImageUri) {
      return;
    }
    
    setShowBankDetailsModal(false);
    uploadReceipt(selectedImageUri);
  };
  ...
  const uploadReceipt = async (uri: string) => {
    ...
    try {
      ...
      try {
        ...
        // Save receipt info to Firestore with bank details
        const receiptRef = await addDoc(collection(db, 'vouchers'), {
                ...
                <View style={styles.bankDetailsSection}>
                  ...
                  {selectedImage.amount && (
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.amountValue}>
                        ${typeof selectedImage.amount === 'number' 
                          ? selectedImage.amount.toLocaleString() 
                          : selectedImage.amount}
                      </Text>
                    </View>
                  )}
                  
                  {selectedImage.bankName && (
                    <View style={styles.detailRow}>
                      <Text style={styles.detailLabel}>Bank:</Text>
                      <Text style={styles.detailValue}>{selectedImage.bankName}</Text>
                    </View>
                  )}
                  
                  {selectedImage.accountTitle && (
                    <View style={styles.detailRow}>
                      <Text style={styles.detailLabel}>Account Title:</Text>
                      <Text style={styles.detailValue}>{selectedImage.accountTitle}</Text>
                    </View>
                  )}
                  ...
                  {selectedImage.accountNumber && (
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailValue}>{selectedImage.accountNumber}</Text>
                    </View>
                  )}
                </View>
        ...
        <View style={styles.modalContent}>
          ...
          <ScrollView style={styles.modalBody}>
            <Text style={styles.bankDetailsInfo}>
              Please provide bank details and payment amount for your receipt.
            </Text>
            
            <Text style={styles.inputLabel}>Amount</Text>
            <TextInput
              style={[styles.input, bankFormErrors.amount && styles.inputError]}
              placeholder="Enter amount"
              value={bankDetails.amount}
              onChangeText={(text) => {
                setBankDetails({...bankDetails, amount: text});
                if (bankFormErrors.amount) {
                  setBankFormErrors({...bankFormErrors, amount: undefined});
                }
              }}
              keyboardType="decimal-pad"
            />
            {bankFormErrors.amount && <Text style={styles.errorText}>{bankFormErrors.amount}</Text>}
            
            <Text style={styles.inputLabel}>Bank Name</Text>
            <TextInput
              style={[styles.input, bankFormErrors.bankName && styles.inputError]}
              placeholder="Enter bank name"
              value={bankDetails.bankName}
              onChangeText={(text) => {
                setBankDetails({...bankDetails, bankName: text});
                if (bankFormErrors.bankName) {
                  setBankFormErrors({...bankFormErrors, bankName: undefined});
                }
              }}
            />
            {bankFormErrors.bankName && <Text style={styles.errorText}>{bankFormErrors.bankName}</Text>}
            
            <Text style={styles.inputLabel}>Account Title</Text>
            <TextInput
              style={[styles.input, bankFormErrors.accountTitle && styles.inputError]}
              placeholder="Enter account title"
              value={bankDetails.accountTitle}
              onChangeText={(text) => {
                setBankDetails({...bankDetails, accountTitle: text});
                if (bankFormErrors.accountTitle) {
                  setBankFormErrors({...bankFormErrors, accountTitle: undefined});
                }
              }}
            />
            {bankFormErrors.accountTitle && <Text style={styles.errorText}>{bankFormErrors.accountTitle}</Text>}
            
            <Text style={styles.inputLabel}>Account Number</Text>
            <TextInput
              style={[styles.input, bankFormErrors.accountNumber && styles.inputError]}
              placeholder="Enter account number"
              value={bankDetails.accountNumber}
              onChangeText={(text) => {
                setBankDetails({...bankDetails, accountNumber: text});
                if (bankFormErrors.accountNumber) {
                  setBankFormErrors({...bankFormErrors, accountNumber: undefined});
                }
              }}
              keyboardType="number-pad"
            />
            {bankFormErrors.accountNumber && <Text style={styles.errorText}>{bankFormErrors.accountNumber}</Text>}
            
            <TouchableOpacity 
              style={styles.submitButton}
              onPress={submitBankDetailsAndUpload}
            >
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.uploadButtonContainer}>
        ...
        >
          ...
          <Text style={styles.makePaymentText}>Make Payment Request</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
